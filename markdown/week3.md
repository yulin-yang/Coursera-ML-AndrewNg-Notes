...
在线性回归中我们的假设函数 $h_\theta(x)=\theta^Tx$ ，现在做一些小改变，令  $h_\theta(x)=g(\theta^Tx)$，其中 $g(z)=\frac{1}{1+e^{-z}}$

![1679238801889](assets/1679238801889.png)
...
$h_\theta \left( x \right)$的作用是，对于给定的输入变量，根据选择的参数计算输出变量=1的可能性（**estimated probability**）即
...
![1679239321799](assets/1679239321799.png)
...
现在假设我们有一个模型 ，并且假设已知参数$\theta$ 是向量 [-3 1 1]
...
![](assets/1679241039035.png)

因为需要用曲线才能分隔 $y=0$ 的区域和 $y=1$ 的区域，我们需要二次方特征：

我们令 ${h_\theta}\left( x \right)=g\left( {\theta_0}+{\theta_1}{x_1}+{\theta_{2}}{x_{2}}+{\theta_{3}}x_{1}^{2}+{\theta_{4}}x_{2}^{2} \right)$

其中 $\theta$ 是[-1 0 0 1 1]，则我们得到的判定边界恰好是圆点在原点且半径为1的圆形。
...
![Want ${{\min }_\theta}J(\theta )$：](assets/171031235527.png)
