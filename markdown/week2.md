...
解决的方法是尝试将所有特征的尺度都尽量缩放到-1到1之间。例如我们做如下操作：
...
![1679069471520](assets/1679069471520.png)
...
- 如果梯度下降算法正常工作的话，那么在 $J\theta$ 应该在每一次迭代都下降

![](assets/1679070138239.png)

如图所示在 $300$ ~ $400$ 次迭代时曲线已经趋于水平，所以我们可以判断在 $400$ 次迭代后收敛。 也有一些自动测试是否收敛的方法，例如将代价函数的变化值与某个阀值（例如0.001）进行比较，但选择一个合适的阈值是十分困难的，所以通常看上面这样的图表更好。
...
那么我们如何将模型与数据进行拟合呢？通过使用多元线性回归，我们可以对算法做一个简单的修改来实现

![1679071599646](assets/1679071599646.png)

上面这幅图我们初次看到可能觉得使用二次函数进行拟合，但二次函数最终会降下来，不可能随着 size 的增大反而价格减小，所以我们使用三次函数。自然地想到令 
